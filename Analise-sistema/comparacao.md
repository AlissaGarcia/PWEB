...
As classes definidas para o sistema foram escolhidas com base na análise do comportamento visível da aplicação, mesmo sem acesso ao seu backend. A interface do sistema permite identificar claramente um fluxo de pedidos típico de uma pizzaria, o que possibilita inferir as principais entidades envolvidas. A classe Produto foi definida por representar os itens exibidos no cardápio, como pizzas, hambúrgueres e bebidas, sendo um elemento central do sistema. Já a classe Categoria foi incluída para organizar esses produtos, conforme observado na separação por grupos como “Produtos”, “Combos” e “Promoções”, melhorando a estrutura e navegação. A classe Pedido foi identificada a partir da presença do carrinho de compras e do processo de finalização, indicando que o sistema registra as solicitações realizadas pelos clientes. Para detalhar os itens dentro de um pedido, foi criada a classe ItemPedido, responsável por armazenar informações como produto selecionado, quantidade e observações, refletindo o funcionamento típico de sistemas de pedidos. A classe Cliente foi considerada necessária devido à existência de informações como contato e endereço para entrega, visíveis na proposta do sistema, mesmo que não diretamente exibidas na interface inicial.